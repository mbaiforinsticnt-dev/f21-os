...
- `rom/assemble-package.sh` runs the full assembly: verifies the locked GSI checksum, applies the F21 patches and the debloat list to the image, injects tt9 plus the launcher plus the allowlist, and refuses to package unless every payload verifies inside the patched image.
